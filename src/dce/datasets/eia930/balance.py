...
def compute_balance_residuals(
    ...
    """
    df_res = df.copy()
    calculated_demand = df_res["generation"] + df_res["interchange"]
    df_res["accounting_residual"] = calculated_demand - df_res["demand"]
...
def validate_physical_balance(
    ...
    Args:
        ...
        reconcile_anomalies: Whether to adjust interchange for extreme outliers.
    ...
    if reconcile_anomalies and anomalies.any():
        # Reconcile interchange to close the physical gap on flagged reporting anomalies
        df_clean.loc[anomalies, "interchange"] = (
            df_clean.loc[anomalies, "demand"] - df_clean.loc[anomalies, "generation"]
        )
