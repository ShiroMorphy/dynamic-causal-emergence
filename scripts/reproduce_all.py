...
def ensure_synthetic_benchmarks(from_scratch: bool = False) -> bool:
    ...
    # Bandwidth MUST be 24.0 here: Table 1 and every results/synthetic/dgp_*.json
    # artifact was produced at h=24, and the manuscript reports those benchmarks as
    # h=24 (N_eff ~ 85; DGP-D bias -2.113 under h=24). The h=36 budget belongs to the
    # baseline comparison of Table 2 and to DGP-K, which are generated by
    # scripts/generate_baseline_comparison.py, not here. Running this stage at 36.0
    # silently overwrites the published Table 1 with numbers that contradict the paper
    # (e.g. DGP-A mean_rmse_dcd_pr 2.464 -> 1.227, bias -2.306 -> -0.980).
    res = subprocess.run([
        sys.executable,
